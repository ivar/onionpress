...
Extract and write the onion service's Ed25519 expanded private key to/from
the onionpress-tor container. The delivered key is an OpenSSH-format PEM (the
format arti introduced; OnionHeaven kept it as its wire format) in the
onionpress-onion-keys volume; C Tor's own key files are what Tor runs on.
"""
...
# The delivered PEM, in the onionpress-onion-keys volume as the container sees
# it. Renamed on 2026-09-25 from /var/lib/arti/state/keystore/hss/... (volume
# onionpress-arti-state); the launchers migrate old volumes on first start.
DELIVERED_KEY_DIR = "/var/lib/onionpress-keys/wordpress"
DELIVERED_KEY_PATH = DELIVERED_KEY_DIR + "/ks_hs_id.ed25519_expanded_private"
ARTI_KEY_TYPE = b"ed25519-expanded@spec.torproject.org"
CONTAINER = "onionpress-tor"

# C Tor onion service key files — where the running identity lives. The PEM
# above is how the key is *delivered* (launchers write it there; the container
# converts it on first start); these are what Tor actually reads. Format:
# 32-byte ASCII header + raw ed25519 material.
...
def extract_keys():
    ...
    Tries the delivered PEM first (single file in the onionpress-onion-keys
    volume). Falls back to C Tor's pair of raw header+payload files, which
    is all an install has when the key was generated by Tor itself. If both
    are missing we surface the PEM error, which is the more informative of
    the two on a broken install.
    """
    pem_ok, pem_data = _docker_cat(DELIVERED_KEY_PATH)
    if pem_ok:
        try:
            return parse_openssh_key(pem_data)
        except Exception as e:
            raise Exception(f"Failed to parse delivered PEM key: {e}")
    ...
    # Neither layout was readable. Report the PEM failure (it's what
    ...
    detail = pem_data.decode(errors="replace").strip() if isinstance(pem_data, bytes) else str(pem_data)
    raise Exception(f"Failed to extract keys: Could not read key file: {detail}")


def extract_private_key():
    """
    Extract the Ed25519 expanded private key from the container.
...
def extract_public_key():
    """
    Extract the Ed25519 public key from the container.
...
def write_private_key(private_key, public_key):
    """
    Write a new key pair into the container's key volume as an OpenSSH PEM
    and restart the tor container. This will change your onion address!
    """
    import tempfile
    import os
    ...
    try:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "exec", CONTAINER, "mkdir", "-p", DELIVERED_KEY_DIR],
                capture_output=True,
                timeout=10
            )
            ...
            result = subprocess.run(
                ["docker", "cp", temp_path,
                 f"{CONTAINER}:{DELIVERED_KEY_PATH}"],
                capture_output=True,
                timeout=10
            )
            if result.returncode != 0:
            ...
            result = subprocess.run(
                ["docker", "exec", CONTAINER, "chmod", "600", DELIVERED_KEY_PATH],
                capture_output=True,
                timeout=10
            )
            if result.returncode != 0:
